...
from .hermes.tools import ChromeExtensionWebTools, DirectWebTools, HermesCLITools
...
def build(settings: Settings) -> NewsAgent:
    ...
    tool_instance = tools(settings) if settings.tool_backend == "hermes" else tools()
...
def doctor(settings: Settings) -> int:
    checks={"python":sys.version.split()[0],"model_backend":settings.model_backend,"model":settings.model_name,"tool_backend":settings.tool_backend,
            "hermes_cli":shutil.which(settings.hermes_command),"ollama_cli":shutil.which("ollama")}
    try:
        response=httpx.get(f"{settings.model_base_url}/api/tags",timeout=3); checks["model_endpoint"]="ok" if response.is_success else f"HTTP {response.status_code}"
        checks["model_installed"]=any(x.get("name","").split(":")[0]==settings.model_name.split(":")[0] for x in response.json().get("models",[]))
    except Exception as exc: checks["model_endpoint"]=f"unavailable: {type(exc).__name__}"
    settings.ensure_dirs(); checks["database_path"]=str(settings.database_path.resolve()); print(json.dumps(checks,indent=2));
    return 0 if checks.get("model_endpoint")=="ok" and (settings.tool_backend!="hermes" or checks["hermes_cli"]) else 1


def main(argv: list[str] | None=None) -> int:
    p=argparse.ArgumentParser(prog="news-agent",description="Local Qwen + Hermes news research agent")
    p.add_argument("--tools",choices=["hermes","direct","fixture","extension","chrome"],help="override configured tool backend")
